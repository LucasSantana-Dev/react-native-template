...
import { HomeVirtualizedItem } from './components/home-virtualized-item';
...
export default function HomeScreenVirtualized() {
  const { listData, refreshing, isTablet, colors, handleRefresh, handleSignOut } = useHomeData();

  const renderItem = ({ item }: { item: HomeItem }) => (
    <HomeVirtualizedItem item={item} isTablet={isTablet} colors={colors} onQuickAction={() => {}} />
  );
